const getMessage = async () => {
  ...
  try {
    const response = await fetch(
      'https://raw.githubusercontent.com/tconey23/connect21_be/refs/heads/main/ServerData/gptResp.json'
    );
    ...
    setData(result.choices[0].message.content);  // Adjust based on your JSON structure
    setLoading(false);
  } catch (err) {
    setError(err);
    setLoading(false);
